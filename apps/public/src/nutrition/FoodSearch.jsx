 ...
 * le sélecteur d'ingrédients d'une recette. CIQUAL tourne à chaque caractère, OFF seulement
 * 700 ms après la dernière frappe — son quota de recherche texte est trop étroit pour
 * suivre la frappe (voir off.js).
 */
...
// Bloc "Produits industriels" affiché sous les résultats CIQUAL.
...
// Une valeur absente de la table n'est pas un zéro : on l'affiche « — ».
...
// Répare un ingrédient de recette créé sans per100 stocké (voir CLAUDE.md, V8 côté
// apps/perso) : re-résout depuis la source d'origine (CIQUAL/OFF) via le ref.
...
function IngredientPicker({ onAdd, onCancel, log, pins, muted }) {
  ...
  // Favoris/aliments habituels (07/08/2026, port depuis apps/perso) : IngredientPicker
  // n'affichait QUE la recherche textuelle — pas de `meal` ici (un ingrédient de recette
  // n'est pas lié à un repas précis).
...
function RecipeBuilder({ recipe, onSave, onBack, log, pins, muted }) {
  ...
  // Auto-réparation : recette créée sans per100 stocké par ingrédient.
  useEffect(() => {
