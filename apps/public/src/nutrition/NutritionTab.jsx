// Onglet Repas — RawCare Phase 2, dixième jalon (dernier écran de données d'apps/public).
// Port depuis apps/perso/src/nutrition/NutritionTab.jsx, adapté à la signature commune des
...
// "Copier un repas" : liste des jours qui ont déjà ce repas rempli, tap = copie immédiate.
...
// Dupliquer la journée affichée vers une autre date. Fusion, jamais d'écrasement :
// copyEntries ne fait qu'AJOUTER des lignes.
...
export default function NutritionTab({ data, update, error: loadError }) {
  ...
  const food = useFoodLog(data, update);

  const dayEntries = useMemo(() => entriesFor(food.log, date), [food.log, date]);
  const t = useMemo(() => totals(dayEntries), [dayEntries]);

  const tg = targetsForDate(date, targets);
  const kcalTarget = Math.round(kcalFromMacros(tg.protein, tg.carbs, tg.fat, tg.fiber));
  const left = kcalTarget - t.kcal;

  // Eau : PAS une donnée du module Nutrition (voir CLAUDE.md) — même compteur que l'onglet
  // Macros, écrit directement dans macroLog.water sans passer par useFoodLog.
  ...
  // Navigation au doigt : swipe gauche = jour suivant, droite = jour précédent. Décision au
  // relâché (touchend), jamais preventDefault sur touchmove — le scroll vertical normal
  // n'est donc jamais bloqué. Départ ignoré près du bord gauche (geste "retour" système).
  const touchStart = useRef(null);
  const EDGE_GUARD = 24;
  const SWIPE_MIN_DIST = 60;
  const SWIPE_MAX_SLOPE = 0.6;
  const onTouchStart = (e) => {
    if (openMeal) { touchStart.current = null; return; }
    const tt = e.touches[0];
    touchStart.current = tt.clientX < EDGE_GUARD ? null : { x: tt.clientX, y: tt.clientY };
  };
  const onTouchEnd = (e) => {
  ...
  // Barre collante : repère le jour affiché même après un scroll.
  const dayDiff = (key) => {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}
      ...
      {diff !== 0 && (
        <div style={{
          ...
          margin: "0 -24px", padding: "9px 24px",
      ...
      <Body style={{ fontSize: 10, color: C.dim, textAlign: "center", padding: "4px 0 2px" }}>
        Module Repas — calories et macros alimentent l'onglet Macros.
      </Body>

      {(food.error || loadError) && <p style={{ color: C.danger, fontSize: 12 }}>{food.error || loadError}</p>}
